...
def draw_flowchart(event=None):
    ...
    # Use the actual measured width from our earlier testing
    # We measured 4.713 units for a shorter line, so use a generous multiplier
    fontsize = 10
    max_chars = 0
    for node in nodes:
        lines = node.split('\n')
    ...
    # Calculate width based on content, but stay within PyScript coordinate limits
    # Use a conservative multiplier that works with the coordinate system
    consistent_width = max_chars * 0.3 + 2.0  # Much more conservative calculation
    node_height = 0.8  # Fixed height for all nodes
    
    # Debug: show the calculated width
    print(f"Max chars: {max_chars}, Calculated width: {consistent_width}")
    ...
    # Show debug info in title
    plt.title(f"Width: {consistent_width:.1f} | Chars: {max_chars}", fontsize=8, pad=10)
    plt.show()
